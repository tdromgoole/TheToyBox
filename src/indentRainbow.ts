...
const rainbowColors = [
	'#FF6B6B', '#4ECDC4', '#ff9900', '#9B59B6', '#eeff00', '#45B7D1',
	'#E74C3C', '#2e46cc', '#52f312', '#451abc', '#C0392B', '#ff2da7'
];
...
export function refreshIndentRainbow() {
	...
	if (isEnabled) {
		indentDecorations = rainbowColors.map(color =>
...
export function updateIndentRainbow(editor?: vscode.TextEditor) {
	...
	const indentRanges: vscode.Range[][] = Array.from({ length: rainbowColors.length }, () => []);
	...
	for (let i = 0; i < activeEditor.document.lineCount; i++) {
		...
		for (let j = 0; j < tabCount; j++) {
			const colorLevel = j % rainbowColors.length;
