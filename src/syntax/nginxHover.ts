...
let _directiveDocs: Map<string, string> | undefined;
function getDirectiveDocs(): Map<string, string> {
	if (_directiveDocs) {
		return _directiveDocs;
	}
	_directiveDocs = new Map<string, string>([
		// ── Main context ──
		[
			"user",
...
let _variableDocs: Map<string, string> | undefined;
function getVariableDocs(): Map<string, string> {
	if (_variableDocs) {
		return _variableDocs;
	}
	_variableDocs = new Map<string, string>([
		// Request
		[
			"request",
			"Full original request line, e.g. `GET /path?query HTTP/1.1`.",
		],
		...
		[
			"is_args",
			"`?` if a request line has arguments, empty string otherwise.",
		],
		[
			"arg_name",
		...
		[
			"server_name",
			"Name of the virtual server that received the request.",
		],
		...
		[
			...
			"The `Cache-Control` response header field.",
		],
		[
			"sent_trailer_name",
		...
		[
			"document_root",
			"Root or alias directive value for the current request.",
		],
		[
			"realpath_root",
		...
		[
			"proxy_port",
			"Port of the proxied server as specified in `proxy_pass`.",
		],
		[
			"upstream_addr",
		...
		[
			"ssl_session_id",
			"Session identifier of an established TLS connection.",
		],
					...
					return new Promise((resolve) => {
						...
						const timer = setTimeout(() => {
							...
							if (word.startsWith("$")) {
								...
								const doc = getVariableDocs().get(varName);
								if (!doc) {
									return resolve(undefined);
								}
							...
							const doc = getDirectiveDocs().get(lower);
							if (!doc) {
								return resolve(undefined);
							}
							const md = new vscode.MarkdownString(
