...
export function getCleanEdits(
	...
	const ignoredExtensions = config.get<string[]>('ignoreTabConversion', [".yaml", ".yml", ".json"]);
	...
	// Auto-detect the indentation unit by finding the smallest indentation
	...
	for (let i = 0; i < document.lineCount; i++) {
		...
		if (leadingMatch && leadingMatch[0].length > 0) {
			minIndent = Math.min(minIndent, leadingMatch[0].length);
		}
	}

	// Use detected indent or default to 4
	const indentUnit = minIndent === Infinity ? 4 : minIndent;
	...
	for (let i = lineStart; i <= lineEnd; i++) {
		...
		// 2. Convert Leading Spaces to Tabs (Skip if file extension is ignored)
		if (!skipTabConversion) {
			const leadingMatch = line.text.match(/^ +/);
			if (leadingMatch) {
				const spaceCount = leadingMatch[0].length;
				const tabCount = Math.floor(spaceCount / indentUnit);
				const remainingSpaces = spaceCount % indentUnit;

				// Construct the new indentation string
				const newIndentation = '\t'.repeat(tabCount) + ' '.repeat(remainingSpaces);

				// Only create an edit if the indentation actually changed
				if (newIndentation !== leadingMatch[0]) {
					edits.push(vscode.TextEdit.replace(new vscode.Range(i, 0, i, spaceCount), newIndentation));
				}
			}
		}

		// 3. Trim Trailing (Always runs even if tab conversion is skipped)
		const trailingMatch = line.text.match(/[ \t]+$/);
		if (trailingMatch) {
			const startCol = line.text.length - trailingMatch[0].length;
			edits.push(vscode.TextEdit.delete(new vscode.Range(i, startCol, i, line.text.length)));
		}
	}
...
export function registerCleanupCommand(
	...
	let cleanCmd = vscode.commands.registerCommand('theToyBox.cleanFile', async () => {
		...
		try {
			...
			if (editor.selections.length > 1) {
				...
				if (isMultilineSelection) {
					edits = getCleanEdits(editor.document, selection.active.line, startLine, endLine);
				} else {
