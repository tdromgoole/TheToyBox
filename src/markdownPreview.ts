  ...
  md.core.ruler.push("github_alerts", (state: any) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      const html =
        ...
        `<p class="alert-title"><span class="alert-icon">${info.icon}</span> ${info.title}</p>` +
        `<p>${rendered}</p>` +
        `</div>\n`;
...
class CustomMarkdownPreviewProvider {
  ...
  private transformMarkdownToHtml(markdown: string): string {
      ...
      if (inBlockquote) {
        if (trimmed.startsWith(">")) {
          ...
          if (i + 1 < lines.length && lines[i + 1].trim().startsWith(">")) {
            ...
            inBlockquote = false;
            result +=
              this.formatAlert(alertType, blockquoteContent.trim()) + "\n\n";
            alertType = "";
            blockquoteContent = "";
          }
        } else {
    ...
    if (inBlockquote && blockquoteContent) {
      result += this.formatAlert(alertType, blockquoteContent.trim());
    }

    return this.basicMarkdownToHtml(result);
  }

  private formatAlert(type: string, content: string): string {
    ...
    return `<div class="markdown-alert ${type}">\n<p class="alert-title"><span class="alert-icon">${alertInfo.name}</span> ${alertInfo.title}</p>\n<p>${content}</p>\n</div>`;
  }
